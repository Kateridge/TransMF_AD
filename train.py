...
from models.mymodel import model_CNN, model_pretrain, model_transformer, model_ad
...
if __name__ == '__main__':
    ...
    train_dataloader, val_dataloader = get_dataset(opt)
    # torch.save({'train': train_dataloader, 'val': val_dataloader, 'CNN_PET_ADCN': test_dataloader}, 'dataloaders.pt')

    # get networks
    net_model = model_ad(dim=opt.dim, depth=opt.trans_enc_depth, heads=4, dim_head=opt.dim // 4,
                                  mlp_dim=opt.dim * 4,  dropout=opt.dropout).to(device)
    # net_model = model_CNN(dim=opt.dim).to(device)

    # best_model_path = './checkpoints/Transformer_ADCN/best_label_net_model_15_accuracy=0.9533.pt'
