...
from models.mymodel import model_CNN, model_pretrain, model_cl, model_withSupCL
...
if __name__ == '__main__':
    ...
    # load ADNI dataset
    ADNI = ADNI(dataroot='/home/kateridge/Projects/Projects/Datasets/ADNI_SPM',
                label_filename='ADNI.csv', task=opt.task)
    train_transforms, val_transforms = ADNI_transform()
    ADNI_dataset = Dataset(data=ADNI.data_dict, transform=train_transforms)
    ...
    def setup_dataflow(dataset, train_idx, test_idx):
        ...
        val_loader = DataLoader(dataset, batch_size=opt.batch_size, sampler=val_sampler)
    ...
    def init_model(model):
        ...
        if model == 'Transformer':
            net_model = model_withSupCL(dim=opt.dim, depth=opt.trans_enc_depth, heads=4,
    ...
    def train_model(train_dataloader, val_dataloader, test_dataloader, fold, weights):
        ...
        # best_model_filename = glob.glob(os.path.join(save_path_fold, 'best_label_net_model_*.pt'))[0]
        # best_model_filename = best_model_filename.split('/')[-1]
        # best_epoch = int(best_model_filename.split('_')[4])
        # best_metrics = test_status[best_epoch - 1]
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold_splits.split(np.arange(len(ADNI_dataset)))):
        logger_main.print_message(f'************Fold {fold_idx}************')
        train_dataloader, val_dataloader, test_dataloader, weights = setup_dataflow(ADNI_dataset, train_idx, test_idx)
